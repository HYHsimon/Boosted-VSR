...
def make_model(args, parent=False):
    return classVSR_3class_EDVR(args)
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat):
        ...
        # fusion
        if t == 1:
            aligned_feat = aligned_feat.repeat(1, 5, 1, 1)

        if t == 3:
            aligned_feat = torch.cat((aligned_feat[:, 0: 64], aligned_feat, aligned_feat[:, 128: 192]), dim=1)
...
class EDVR(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.with_predeblur:
            ...
            feat_l1 = self.lrelu(self.conv_first(x.contiguous().view(-1, c, h, w)))
...
class classVSR_3class_EDVR(nn.Module):
    def __init__(self,
                 opt,
                 num_in_ch=3,
                 num_out_ch=3,
                 ...
                 with_predeblur=False,
                 with_tsa=True):
        super(classVSR_3class_EDVR, self).__init__()
        self.upscale=4
        # self.classifier=Classifier()
        self.netA = EDVR(num_in_ch,
                 num_out_ch,
                 num_feat,
                 num_frame,
                 deformable_groups,

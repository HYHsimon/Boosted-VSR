...
save_dir = '/opt/tiger/Datasets/VSR/Youku/LR/LR_compress_BD10'
save_tmp_dir = '/opt/tiger/Datasets/VSR/Youku/LR/LR_compress_BD10_tmp'
...
for video in video_list:
    sigma1 = random.uniform(0.3, 2)
    sigma2 = random.uniform(0.3, 1.2)
    degraded_scales = [540, 360, 270]
    degraded_scale = random.choice(degraded_scales)
    bitrate = random.randint(800,2000)

    output_path = os.path.join(save_tmp_dir, video.split('/')[-1][:-4] + '.mp4') 
    cmd_blur = "ffmpeg -y -i {} -c:v libx264 -pix_fmt yuv420p -vf \"gblur=sigma={}\" -crf 1 {}".format(video, sigma1, output_path)
    os.system(cmd_blur)

    input_path = output_path
    output_path = os.path.join(save_dir, video.split('/')[-1][:-4] + '.mp4') 
    cmd_down = "ffmpeg -y -i {} -c:v libx264 -b:v 120000k -vf scale=-1:{} {}".format(input_path, degraded_scale, output_path)
    os.system(cmd_down)

    input_path = output_path
    output_path = os.path.join(save_tmp_dir, video.split('/')[-1][:-4] + '.mp4') 
    cmd_blur2 = "ffmpeg -y -i {} -c:v libx264 -pix_fmt yuv420p -vf \"gblur=sigma={}\" -crf 1 {}".format(video, sigma2, output_path)
    os.system(cmd_blur2)

    input_path = output_path
    output_path = os.path.join(save_dir, video.split('/')[-1][:-4] + '.mp4') 
    cmd_fin = "ffmpeg -y -i {} -c:v libx264 -b:v {}k -vf scale=960:540 {}".format(input_path, bitrate, output_path)
    os.system(cmd_fin)
    # 
    # bitrate = 800

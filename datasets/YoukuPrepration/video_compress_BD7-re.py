...
save_dir = '/opt/tiger/Datasets/VSR/Youku/LR/LR_compress_BD7_re'
save_tmp_dir = '/opt/tiger/Datasets/VSR/Youku/LR/LR_compress_tmp'
...
for video in video_list:
    ...
    input_path = output_path
    output_path = os.path.join(save_dir, video.split('/')[-1][:-4] + '.mp4') 
    cmd_compress = "ffmpeg -y -i {} -c:v libx264 -b:v {}k -vf scale=960:540 {}".format(input_path, bitrate, output_path)
    os.system(cmd_compress)

    # input_path = output_path
    # output_path = os.path.join(save_dir, video.split('/')[-1][:-4] + '.mp4') 
    # cmd_fin = "ffmpeg -y -i {} -c:v libx264 -b:v {}k {}".format(input_path, bitrate, output_path)
    # os.system(cmd_fin)
    # 
    # bitrate = 800

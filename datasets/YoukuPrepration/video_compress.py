...
save_dir = '/opt/tiger/Datasets/VSR/Youku/LR/LR_compress_BD9'
save_tmp_dir = '/opt/tiger/Datasets/VSR/Youku/LR/LR_compress_BD9_tmp'
...
for video in video_list:
    ...
    cmd_down = "ffmpeg -y -i {} -c:v libx264 -b:v 50000k -vf scale=-1:{} {}".format(input_path, degraded_scale, output_path)
    os.system(cmd_down)

    input_path = output_path
    output_path = os.path.join(save_tmp_dir, video.split('/')[-1][:-4] + '.mp4') 
    cmd_up = "ffmpeg -y -i {} -c:v libx264 -b:v 50000k -vf scale=1920:1080 {}".format(input_path, output_path)
    os.system(cmd_up)
